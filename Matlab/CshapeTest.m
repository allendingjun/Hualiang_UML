...
tmpScriptFile = [pwd, '\CshapeTest2.vbs'];

hfssExePath = 'D:\Program Files\AnsysEM\AnsysEM17.1\Win64\ansysedt.exe';

% fid=fopen(tmpScriptFile,'wt'); %% "t" is in text mode

fid=fopen(tmpScriptFile,'w');

hfssNewProject(fid);

hfssInsertDesign(fid,'CshapeTest');

% function CSlot_Cshape(fid,r1,alp1,theta1,w1,r,alp,theta,w,rc,thickness,P,NUM)

CSlot_Cshape(fid,55,60,45,5,25,90,-45,5,35,0.2,120,1);
CSlot_Cshape(fid,55,60,45,5,25,90,-45,5,35,0.2,120,2);

fclose(fid);

% hfssExecuteScript(hfssExePath, tmpScriptFile, false, false);  %% not working
